...
_CKPT_NAMES = {
    'resnet50_se': 'models/ResNet50-SE.npz',
    'vgg_gap': 'models/vgg16.npz',
}
...
class Model(ModelDesc):
    ...
    def inputs(self):
        ...
        return [
            tf.placeholder(tf.uint8, input_shape, 'input'),
    ...
    def build_graph(self, image, label, bbox):
        ...
        if self.args.method_name in ['AAE', 'SAE', 'AAE_SAE']:
            logit, convmap = arch.__dict__[self.args.arch_name](image,
                                                                self.args)
            # tf.identity(maps[0], name='pool1')
            # tf.identity(maps[1], name='pool2')
            # tf.identity(maps[2], name='pool3')
            # tf.identity(maps[3], name='pool4')
            # tf.identity(maps[4], name='conv53')
            self._prepare_cam(logit, convmap, label_onehot)

            logits = (logit[0] + logit[1] + logit[2])
            # loss1 = self._get_loss(logit[0], label)
            # loss2 = self._get_loss(logit[1], label)
            # loss3 = self._get_loss(logit[2], label)
            loss = self._get_loss(logits, label)
        else:
            logit, convmap = arch.__dict__[self.args.arch_name](image,
                                                                self.args)
            self._prepare_cam(logit, convmap, label_onehot)
    ...
    def optimizer(self):
        lr = tf.get_variable('learning_rate',
                             initializer=self.args.base_lr,
                             trainable=False)
        opt = tf.train.MomentumOptimizer(lr, 0.9, use_nesterov=True)
        add_moving_summary(lr)
        if self.args.use_pretrained_model:
            gradprocs = [
                gradproc.ScaleGradient(_LR_SCALE[self.args.arch_name])
            ]
    ...
    def _prepare_cam(self, logits, convmaps, label_onehot):
        if self.args.method_name in ['AAE', 'SAE', 'AAE_SAE']:
            for _i, n in enumerate(['', '_1x3', '_3x1']):
                _, indices = tf.nn.top_k(logits[_i], 5)
                tf.identity(indices, name='top5' + n)
                tf.identity(convmaps[_i], name='actmap' + n)
                y_c = tf.reduce_sum(tf.multiply(logits[_i], label_onehot),
                                    axis=1)
                tf.identity(tf.gradients(y_c, convmaps[_i])[0],
                            name='grad' + n)
        else:
    ...
    def _get_loss(self, logits, label):
        ...
        wd_cost = regularize_cost('.*/W',
                                  l2_regularizer(
                                      _WEIGHT_DECAY[self.args.arch_name]),
    ...
    def _compute_loss_and_error(self, logits, label):
        def prediction_incorrect(logits_,
                                 label_,
                                 topk=1,
...
def get_config(model, option):
    ...
    return TrainConfig(
        ...
        starting_epoch=option.starting_epoch,
    )
